...
% Paul W. Leu
  ...
  for i = 1:size(sr, 2)
    if nargin > 2
      sr(i).ReflectionResults.plot_versus_wavelength(varargin{i}{:});
    else
  ...
  for i = 1:size(sr, 2)
    if nargin > 2
      sr(i).TransmissionResults.plot_versus_wavelength(varargin{i}{:});
    else
  ...
  for i = 1:size(sr, 2)
    if nargin > 2
      sr(i).AbsorptionResults.plot_versus_wavelength(varargin{i}{:});
    else
