...
%output
toc_file = fullfile(LAMPsolar_path,'examples','demos.xml');
demotoc = file2cell(fullfile(LAMPsolar_path,'examples','toc'));

%input
dom = createDemoDom;

for k=1:numel(demotoc)
  fid = fopen(fullfile(LAMPsolar_path,'examples',[demotoc{k},'.m']),'r');
