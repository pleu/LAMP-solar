function [cnc,rgb] = colornames(palette,varargin)
...
if nargin==0
	if nargout
		...
		out = reshape(fnm,1,[]);
		out(2,:) = cellfun(@(p)numel(data.(p).names),fnm,'Uni',0);
		out(3,:) = cellfun(@(p)data.(p).notes,fnm,'Uni',0);
		out(4,:) = cellfun(@(p)data.(p).source,fnm,'Uni',0);
...
if any(idn==0)
	cnNoMatch(pnm,cnc,arg(idn==0))
end
% If any input name matches multiple regexps, pick the best match:
idz = idn>1;
fun = @(a,v)cnPickBest(data.(pnm),cnc,a,v);
idm(idz) = cellfun(fun, arg(idz),idm(idz), 'UniformOutput',false);
% Get palette indices of matched input names:
idx = cellfun(@find,idm);
idx = 1+mod(idx-1,numel(cnc));
%
cnc = cnc(idx,:);
rgb = rgb(idx,:);
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%colornames
function idm = cnPickBest(sub,cnc,gra,idm)
% Pick the closest color name to match the given input name <gra>.
%
assert(all(size(cnc)==size(idm)),'Not implemented for palettes with indices.')
%
gra = regexprep(gra,sub.regexp,sub.replace);
nme = cellfun(@(s)cnEdits(s,gra),cnc(idm));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%cnPickBest
function cnNoMatch(pnm,cnc,gra)
...
[~,idx] = cellfun(@(s)sort(cellfun(@(n)cnEdits(s,n),cnc)),gra,'UniformOutput',false);
lft = cellfun(@(s)sprintf('%-23s ->',s),gra,'UniformOutput',false);
cus = cellfun(@(s,x)cumsum([2+numel(s);4+cellfun('length',cnc(x))]),lft,idx,'UniformOutput',false);
try
	cwh = get(0,'CommandWindowSize');
catch %#ok<CTCH>
	cwh = matlab.desktop.commandwindow.size;
end
eos = @(s)[s(1:end-1),'.'];
% Join names together without exceeding the command window width:
idn = cellfun(@(v)find(v<=cwh(1),1,'last'),cus,'UniformOutput',false);
