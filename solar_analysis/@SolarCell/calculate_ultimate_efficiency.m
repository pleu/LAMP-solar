...
%      absorptionResults.Data, spectrum.Energy,'linear','extrap'); 
  ...
  if bandGapIndex <= 1
    ...
    [sorted, ind] = sort(absorptionResults.Energy(bandGapIndex));
    ultimateEfficiency = Constants.LightConstants.Q*bandGap * ...
      trapz(absorptionResults.Energy(bandGapIndex(ind)), absorptionData.* ...
      photonFluxInterp(bandGapIndex(ind)))/spectrum.PowerDensityUntruncated;
  end
end
