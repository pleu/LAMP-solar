...
%output
toc_file = fullfile(LAMPsolar_path,'help','LAMP-SOLAR','helptoc.xml');
...
make_toc_ug(dom,fullfile(LAMPsolar_path,'help','GettingStarted'),'greenarrowicon');
make_toc_ug(dom,fullfile(LAMPsolar_path,'help','UsersGuide'),'book_mat');
...
make_toc_ug(dom,fullfile(LAMPsolar_path,'examples'),'demoicon');
make_toc_ug(dom,fullfile(LAMPsolar_path,'help','ReleaseNotes'),'notesicon');
...
tocLAMPsolar = toc.getDocumentElement();
tocLAMPsolar.setAttribute('version','2.0');
tocitem.CONTENT = 'LAMP-Solar Toolbox';
tocitem.ATTRIBUTE.target = 'LAMPsolar_product_page.html';
tocitem.ATTRIBUTE.image = 'LAMPsolar_icon.gif';
struct2DOMnode(toc, toc.getFirstChild, tocitem, 'tocitem');


function make_toc_func(toc,Node,domNode)

funcfile = 'LAMPsolarfuncbycat.html';
