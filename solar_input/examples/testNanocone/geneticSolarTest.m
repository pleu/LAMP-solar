...
gaoptions=gaoptimset('CreationFcn',@createFcn,'CrossoverFcn',@crossoversinglepoint,...
    'CrossoverFraction',0.9,'Generations',2,'MutationFcn',{@mutationadaptfeasible,0.01},...
    'PopulationSize',4,'PopulationType','doubleVector',...
    'SelectionFcn',@selectionuniform,'StallGenLimit',5,...
    'TolFun', 1e-10, 'EliteCount', 1, 'Vectorized','off', 'Display', 'diagnose');
...
upperBound = [0; 0; maxPitch];           %lower bound, T > ET
...
function UE = objFnc(x, ConeHeight)
  ...
  rbot = x(2);
  a = x(3);
  runFDTD(a, ConeHeight, rtop, rbot);
