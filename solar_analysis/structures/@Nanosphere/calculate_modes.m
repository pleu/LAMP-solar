function [lambdaResonancesArray] = calculate_modes(ns, TEorTM)
...
nu = 1;
%lambda = linspace(300e-9, 1190e-9, 10000);
kReal = linspace(2*pi./(max(op.Wavelength)*1e-9), 2*pi./(min(op.Wavelength)*1e-9))';
...
%resonancesVar = zeros(nuMax, size(kGuess, 2)+2);
%plot(k(indReal(indConsReal)), eqn(indReal(indConsReal), 1), 'o')
