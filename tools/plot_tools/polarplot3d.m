...
plst = {'mesh','meshc','wire',...
        'surf','surfc','surfn','surfcn','contour','contourf','off'};
...
switch p.plottype
  ...
  case 'contour', contour(Xi,Yi,Zi,p.contourlines);
    axis equal; xlim(xlim*1.05); ylim(ylim*1.05);
    set(gca,'visible','off');
    set(get(gca,'xlabel'),'visible','on');
    set(get(gca,'ylabel'),'visible','on');
    set(get(gca,'title'), 'visible','on');
  case 'contourf'
    [C, h] = contourf(Xi,Yi,Zi,p.contourlines);
    shading flat;
...
fontargs = {'FontName', get(gca, 'FontName'),'FontSize',18};
%fontargs = {'FontName', get(gca, 'FontName'),'FontSize', get(gca, 'FontSize')};
...
if ~isequal(p.axislocation,'off')
   ...
   if sum(isnan(za)) == length(za)
     if isequal(p.axislocation,'surf')
       za = Zi(1,:);                      % vary along edge of surface
     else za = zeros(size(xa)) + polax;        % constant location
     end
   end
...
%       tl = Rmax  * [1.5; 1.03; 1.1];
%       xl = tl * cos(ta) + p.cartorigin(1);
%       yl = tl * sin(ta) + p.cartorigin(2);
