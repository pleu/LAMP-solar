...
make_LAMPsolar_version; %just to be sure

% alphabetic
dom = createDom;
make_func_alphabetic(toc,toc.getFirstChild,dom);
% publish
  out_file = fullfile(LAMPsolar_path,'help','html','mtexfuncbyalphabet.html');
  xslt(dom,'publishLAMP-solar.xsl',out_file);

% category
dom = createDom;
make_func_category(toc,toc.getFirstChild,dom);
% publish
  out_file = fullfile(LAMPsolar_path,'help','html','mtexfuncbycat.html');
  xslt(dom,'publishLAMP-solar.xsl',out_file);
